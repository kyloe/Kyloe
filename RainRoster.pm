...
sub new
	...
	my $conf = new Config::General('rainroster.cnf');
...
sub retrievePlannedRoster
	...
	$mech->follow_link( text => "Roster");
	$mech->follow_link( url_regex => qr/period=M/i );
...
sub login
	...
	$mech->get($self->{'config'}->{'planned'}->{'loginPageUrl'});
	$mech->field('UserName',$self->{'staffid'});
	$mech->field('Password',$self->{'password'});
	# $mech->submit();
	$mech->click_button(name => "submit");
	print "Logged in to ".$mech->base()."\n";
	return 
	}
...
sub logout
	...
	my $self = shift;
	my $mech = shift;
	#$mech->get('http://crewweb.loganair.co.uk/CWP_LogOut.aspx');
	#$mech->submit();
	}
